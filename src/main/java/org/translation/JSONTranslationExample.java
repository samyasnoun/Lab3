...
public class JSONTranslationExample {
    ...
    public String getCountryNameTranslation(String countryCode, String languageCode) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (countryObj.getString("alpha3").equalsIgnoreCase(countryCode)) {
                if (countryObj.has(languageCode)) {
                    result = countryObj.getString(languageCode);
                }
                else {
                    result = "Translation not found, language code: " + languageCode;
                }
                break;
            }
        }
